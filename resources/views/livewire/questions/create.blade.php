<div x-data="{ showsuggestions: false }">
    <x-ts-card class="space-y-4" header="Adicionar pergunta">
        <x-ts-label class="m-2" :label="'Dica: Você pode colocar ## (duplo jogo da velha) no lugar do nome ' .
            \App\Enums\ModelEnum::from($resource)->getWithPrepositionLabel() .
            '.'" />
        ...
        <x-ts-alert color="amber" light close>
            ...
            Coloque o nome {{ \App\Enums\ModelEnum::from($resource)->getWithPrepositionLabel() }} na pergunta e na
            resposta.<br>
            Na resposta, repita um trecho da pergunta para melhor compreensão.
        </x-ts-alert>

        <div class="p-2 bg-gray-100 dark:bg-gray-800 border rounded-lg divide-y text-sm" x-show="showsuggestions"
            x-transition>
            @foreach ($suggestions as $suggestion)
                <div class="flex justify-between items-center py-2">
                    <span>{{ $suggestion['question'] }}</span>
                    <x-ts-button wire:click="removeSuggestion({{ $suggestion['temp_id'] }})" icon="x-mark"
                        color="red" flat sm />
                </div>
            @endforeach
        </div>
